...
\usepackage{hyperref}
\hypersetup{
  pdftitle={VFI Toolkit: Pseudocodes},    % title
...
Several features of VFI Toolkit have their own companion write-ups that go into more detail and include figures. Two that are relevant to the algorithms described here are Divide and Conquer (\href{https://github.com/robertdkirkby/DivideAndConquer}{github.com/robertdkirkby/DivideAndConquer}) and the Grid Interpolation Layer (\href{https://github.com/robertdkirkby/GridInterpolationLayer}{github.com/robertdkirkby/GridInterpolationLayer}); each github repository contains the corresponding pdf.
...
\begin{algorithmic}
 ...
 \State Evaluate $GeneralEqmConditions=[GEeqn_1(AggVars_1,\theta_1^k),$ $GEeqn_2(AggVars_2,\theta_2^k),$$\dots,$ $GEeqn_T(AggVars_T,\theta_T^k)]$ (evaluate general eqm eqns at each time period)
...
It is important to solve backward for policy and forward for agent distribution. For convenience, aggregate variables and general eqm conditions are often solved forwards while doing agent distribution, but in principle how we deal with these (in terms of timing) is not important in most models (can be important if we use lagged/leading parameter values for something).
